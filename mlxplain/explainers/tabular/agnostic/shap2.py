...
class Shap2Tabular(TabularExplainer):
    """
    The SHAP explainer for tabular data.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.ignored_features = (
            set(ignored_features) if ignored_features is not None else set()
        )
        if self.target_column is not None:
            assert (
                self.target_column not in self.ignored_features
        ...
        self.background_data = shap.sample(
            self.data, nsamples=kwargs.get("nsamples", 100)
        )
        self.explainer = shap.KernelExplainer(
            self.predict_fn, self.background_data, link=self.link, **kwargs
        )
    ...
    def explain(self, X, y=None, **kwargs) -> FeatureImportance:
        ...
        if len(self.ignored_features) == 0:
            ...
            for i, instance in enumerate(instances):
                df = X.iloc(i).to_pd()
                feature_values = [
                    df[self.feature_columns[feat]].values[0]
                    for feat in range(len(self.feature_columns))
                ]
                if self.mode == "classification":
                    label = y[i]
            ...
            for i, instance in enumerate(instances):
                ...
                explainer = shap.KernelExplainer(
                    predict_function,
                    self.background_data[:, self.valid_indices],
                    link=self.link,
                    **kwargs,
                )
                shap_values = explainer.shap_values(
                    np.expand_dims(test_x, axis=0), **kwargs
                )

                df = X.iloc(i).to_pd()
                feature_values = [
                    df[self.feature_columns[f]].values[0] for f in self.valid_indices
                ]
