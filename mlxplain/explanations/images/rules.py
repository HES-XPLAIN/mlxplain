...
class RuleImportance(ExplanationBase):
    def __init__(self, mode: str = "classification", explanations=None, target_class: str = None):
        super().__init__()
        self.mode = mode
    ...
    def _format_rule(self, rule, label, rank):
        conditions = [f"feature {cond.split()[0]} {cond.split()[1]} {cond.split()[2]}" for cond in rule]
        prediction = f'"{self.target_class}"' if label == 1 else f'Not "{self.target_class}"'
    ...
    def plot(self, font_size=10):
        ...
        ax.axis('off')
        table = ax.table(cellText=data.values, colLabels=data.columns, loc='center', cellLoc='left')
        table.auto_set_font_size(False)
        table.set_fontsize(font_size)
        table.scale(1.2, 1.5)
        plt.title('Rule Importance Explanation (Ranked)', fontsize=font_size+4, fontweight='bold', pad=20)
        plt.tight_layout()
        return fig, ax
