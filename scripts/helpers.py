...
def get_dataloaders(batch_size=16):
    ...
    # val_loader = DataLoader(dataset=valid_dataset, batch_size=1, shuffle=True)
...
def load_images_to_ndarray(folder):
    ...
    data = np.stack(image_list, axis=0) if image_list else np.array([])  # Handle empty case
    return data
